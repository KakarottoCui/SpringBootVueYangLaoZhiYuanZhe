    ...
    <a-card :bordered="false" class="card-area">
        ...
        <div>
            <div class="operator" style="width: auto">
                <a-dropdown v-hasPermission="['oldSketch:export']">
                ...
                <a-button v-hasPermission="['oldSketch:batchDelete']" @click="batchDelete">删除</a-button>
                <a-button type="primary" @click="addItem" v-hasPermission="['oldSketch:add']">登记造册</a-button>
                <ImportExcel v-hasPermission="['oldSketch:importData']" ref="ImportExcel"
                             :importUrl="importUrl"
            ...
            <a-table ref="TableInfo"
                ...
                <template slot="operation" slot-scope="text, record">
                    <a-icon style="font-size: 20px" v-hasPermission="['oldSketch:edit']" type="edit" theme="twoTone" twoToneColor="#4a9ff5" @click="itemEdit(record)" title="修改"></a-icon>
                    <a-icon style="font-size: 20px"  type="eye" theme="twoTone" twoToneColor="#42b983" @click="view(record)" title="数据详情"></a-icon>
                </template>
                <!--数据列动态选择渲染-->
<!--                <template slot="filterDropdown" slot-scope="text, record">
                ...
                <template slot="imageTags" slot-scope="text, record">
                    <img :src="record.surfaceImage" @click="handleTagImgChange(record.surfaceImage)" style="width: 30px;height: 30px" />
                </template>
                <template  slot="nursingWorkerNumTags" slot-scope="text, record">
                  <a-tag color="blue">{{handleTypeByValue(record.nursingWorkerNum,authUserIdOption)}}</a-tag>
                </template>
                <template  slot="bedNumTags" slot-scope="text, record">
                  <a-tag color="blue">{{handleTypeByValue(record.bedNum,allBedSketchOption)}}</a-tag>
                </template>
                <template  slot="genderTags" slot-scope="text, record">
                  <a-tag color="blue">{{handleTypeByValue(record.gender,genderOption)}}</a-tag>
                </template>
            </a-table>
        </div>
...
export default {
    ...
    data () {
        return {
            ...
            pageUrl:'api/v1/oldSketch/queryByPage',//分页查询链接
            exportUrl:'api/v1/oldSketch/exportData', //导出链接
            acceptFile:'application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            importUrl:process.env.BASE_API+'api/v1/oldSketch/importData', //文件时上传,
            itemData:{ //行信息
                data:{},
            ...
            dataColumns:[
                ...
                },
                {
                    title: '性别',
                    dataIndex: 'gender',
                    scopedSlots: { customRender: 'genderTags' },
                    showStatus:true,
                    disabled:false,
                },
                {
                ...
                {
                    title: '志愿者',
                    dataIndex: 'nursingWorkerNum',
                    scopedSlots: { customRender: 'nursingWorkerNumTags' },
                    showStatus:true,
                    disabled:false,
                },
                {
            ...
            this.$confirm({
                ...
                onOk () {
                    ...
                    for (let key of that.selectedRowKeys) {
                        mainIdList.push(that.dataSource[key].oldSketchId)
                    }
                    that.$delete('api/v1/oldSketch/batchDeleteItem/' + mainIdList.join(',')).then(() => {
