  ...
  <a-modal
    ...
    <a-form :autoFormCreate="(form)=>{this.form = form}">
      <a-form-item
        ...
        :fieldDecoratorOptions="{rules: [{ required: true, message: '请输入旧密码'}, { validator: this.handleOldPassword }], validateTrigger: ['blur']}">
  ...
  methods: {
    ...
    handleUpdatePassword () {
      this.form.validateFields((err, values) => {
        if (!err) {
          this.$get('api/v1/authUser/updatePassword', {
    ...
    handleOldPassword (rule, value, callback) {
      ...
      if (this.oldPassword.trim().length) {
        this.$get('api/v1/authUser/checkOldPassword', {
          password: password,
          userName: this.user.userName
        }).then((r) => {
          let data = r.data;
