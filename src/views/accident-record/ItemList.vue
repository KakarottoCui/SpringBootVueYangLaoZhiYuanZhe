    ...
    <a-card :bordered="false" class="card-area">
        ...
        <div>
            <div class="operator" style="width: auto">
                <a-dropdown v-hasPermission="['accidentRecord:export']">
                ...
                <a-button v-hasPermission="['accidentRecord:batchDelete']" @click="batchDelete">删除</a-button>
                <a-button type="primary" @click="addItem" v-hasPermission="['accidentRecord:add']">发布服务</a-button>
                <ImportExcel v-hasPermission="['accidentRecord:importData']" ref="ImportExcel" :importUrl="importUrl"
            ...
            <a-table ref="TableInfo" :columns="dataColumns" :dataSource="dataSource" :pagination="pagination"
                ...
                <template slot="operation" slot-scope="text, record">
                    <a-icon v-if="record.status==1" style="font-size: 20px" v-hasPermission="['accidentRecord:edit']"
                        type="edit" theme="twoTone" twoToneColor="#4a9ff5" @click="itemEdit(record)" title="修改">
                    </a-icon>
                    <a-icon style="font-size: 20px" type="eye" theme="twoTone" twoToneColor="#42b983"
                        @click="view(record)" title="数据详情"></a-icon>
                    <a-icon v-if="record.status==1" twoToneColor="#f50" title="响应" type="plus-square" theme="twoTone"
                        @click="receving(record)"></a-icon>
                    <a-icon v-if="record.status==2" twoToneColor="#f5222d" title="结束" type="check-circle"
                        theme="twoTone" @click="finish(record)"></a-icon>
                    <a-icon v-if="record.status==1" twoToneColor="#f50" title="取消" type="close-circle" theme="twoTone"
                        @click="cancel(record)"></a-icon>
                </template>
                <!--数据列动态选择渲染-->
<!--                <template slot="filterDropdown" slot-scope="text, record">
                ...
                <template slot="oldSketchTags" slot-scope="text, record">
                    <a-tag color="blue">{{handleTypeByValue(record.oldSketchId,allOldSketchOption)}}</a-tag>
                </template>
                <template slot="status" slot-scope="text, record">
                    <a-tag :color="record.status === 9 ? 'volcano' : record.status > 3 ? 'geekblue' : 'green'">
                        {{handleTypeByValue(record.status,allStatus)}}
                    </a-tag>
                </template>
                <template slot="operatorTags" slot-scope="text, record">
                    <a-tag color="blue">{{handleTypeByValue(record.operatorId,authUserIdOption)}}</a-tag>
                </template>
            </a-table>
        </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                pageUrl: 'api/v1/serviceRecord/queryByPage', //分页查询链接
                exportUrl: 'api/v1/serviceRecord/exportData', //导出链接
                acceptFile: 'application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                importUrl: process.env.BASE_API + 'api/v1/serviceRecord/importData', //文件时上传,
                recevingUrl: 'api/v1/serviceRecord/receiving/', //接单链接
                finishUrl: 'api/v1/serviceRecord/finish/', //结束订单
                cancelUrl: 'api/v1/serviceRecord/cancel/', //取消订单
                itemData: { //行信息
                    data: {},
                ...
                dataColumns: [{
                    ...
                    {
                        title: '老人姓名',
                        dataIndex: 'oldSketchId',
                        scopedSlots: {
                            customRender: 'oldSketchTags'
                        },
                        showStatus: true,
                        disabled: false,
                    },
        ...
        methods: {
            ...
            receving(record) {
                this.loading = true;
                this.$putBodyRequest(this.recevingUrl + record.serviceRecordId).then(() => {
                    this.loading = false;
                    this.search();
                    this.$message.success('接单成功，请按时服务！')
                }).catch(() => {
            ...
            finish(record) {
                this.loading = true;
                this.$putBodyRequest(this.finishUrl + record.serviceRecordId).then(() => {
                    this.loading = false;
                    this.search();
            ...
            cancel(record) {
                this.loading = true;
                this.$putBodyRequest(this.cancelUrl + record.serviceRecordId).then(() => {
                    this.loading = false;
                    this.search();
                ...
                this.$confirm({
                    ...
                    onOk() {
                        ...
                        for (let key of that.selectedRowKeys) {
                            mainIdList.push(that.dataSource[key].serviceRecordId)
                        }
                        that.$delete('api/v1/serviceRecord/batchDeleteItem/' + mainIdList.join(',')).then(
                            () => {
